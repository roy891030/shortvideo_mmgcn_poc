...
\citet{zhu2020mmved} 的 MMVED 直接從影片內容特徵（視覺、音訊、文字、社交）
預測影片人氣（popularity；以 Vine 平台上的留言數、轉發數、
按讚數、迴圈播放數的穩定化綜合評分衡量），不依賴使用者互動記錄。
在 NUS micro-video 資料集（186,637 部 Vine 短影音）上，
MMVED-REG 的 nMSE = \textbf{0.914}，
優於 ELM（0.982）、TMALL（0.979）與 TRLMVR（0.934）（原論文 Table~2）。
\citet{xie2021hmmved} 進一步引入層次多任務框架 HMMVED，
同時預測多個時序人氣相關指標。這兩項工作\textbf{不依賴使用者互動記錄}，
