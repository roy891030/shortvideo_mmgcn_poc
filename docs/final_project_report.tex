...
優於非圖基準模型 LightGBM 的 0.047，提升 4.4 倍，
並驗證了端到端無歷史資料推論的可行性。
\end{sloppypar}
...
所有協同過濾方法均需要影片在\textbf{訓練集中至少出現一次}，
...
\textbf{無歷史資料影片}（訓練期未見）& \textbf{14,793} & 任何 CF 方法均無法預測 \\
\midrule
...
這是 BM3 的最佳 Recall@10 只有 2.38\% 的根本原因——
協同過濾在稀疏資料上的天花板很低。
\end{definebox}

\subsubsection{侷限三：7 種行為信號的浪費}

資料集最大的貢獻之一是同時記錄了 7 種細粒度行為。
然而，所有基準推薦算法都只把「是否有互動」作為訓練信號，
\textbf{完全忽略}了行為類型的差異。
\texttt{like\_rate} 反映社群認同，\texttt{effective\_view\_rate} 反映主動選擇，
\texttt{mean\_watch\_time} 直接反映內容吸引力——
三者的預測難度與信號來源截然不同，應該分開對待。

\subsection{研究問題的精確定義}

\begin{keybox}[研究問題（Research Question）]
...
LightGCN 透過圖結構傳遞二跳、三跳以上的高階協同信號，
在稀疏資料上顯著優於 BPR（Recall@10：0.0223 vs 0.0113）。
但其本質仍是協同過濾——新影片沒有邊，
訊息傳遞無法作用於它，上架前表現預測問題依舊無解。

\subsection{多模態圖推薦：MMGCN 與 BM3}

\textbf{MMGCN} \citep{wei2019mmgcn} 為每個模態 $m\in\{v,t,b\}$
（視覺、文字、行為）各建立一張使用者–影片圖，
並以 GCN 分別傳播後加權融合：
\begin{equation}
  \mathbf{e}_u = \sum_{m\in\{v,t,b\}}
...
\citet{xie2020mmved} 的 MMVED 直接從影片內容特徵（視覺、音訊、文字）
預測人氣（popularity），是與本研究任務最接近的對標基準模型。
\citet{xie2021hmmved} 進一步引入層次多任務框架 HMMVED，
同時預測多個人氣相關指標。這兩項工作\textbf{不依賴使用者互動記錄}，
與本研究的核心設計哲學一致，是未來比較的主要對象。

\subsection{異質圖神經網路：GraphSAGE 與 HGT}

推薦系統的 GNN（LightGCN、MMGCN）都建立在\textbf{同質圖}上（節點類型相同），
而本研究的圖同時含有「影片」、「作者」、「分類」三種節點，
必須使用能處理多種節點與邊類型的\textbf{異質圖神經網路}。

\subsubsection{GraphSAGE：歸納式均值聚合}
...
及各自反向邊，共 5 種）獨立傳遞信號，不同語意的鄰域不會互相干擾。

\subsubsection{HGT：異質注意力聚合}

\citet{hu2020heterogeneous} 的 HGT 以\textbf{多頭注意力}取代均值，
讓模型自動學習「哪個鄰居更值得參考」：
\begin{equation}
  \mathbf{h}_v^{(\ell+1)} =
  \sigma\!\Biggl(
    ...
    \sum_{u\in\mathcal{N}_r(v)}
      \alpha_{u\to v}^{(r,\ell)} \cdot
      \mathbf{W}_r^{V,(\ell)} \mathbf{h}_u^{(\ell)}
  \Biggr)
  \label{eq:hgt_review}
\end{equation}
注意力係數由查詢（Query）與鍵（Key）的縮放點積計算：
\begin{equation}
  \alpha_{u\to v}^{(r,\ell)} \propto
  \exp\!\left(
    \frac{
  ...
  \right)
  \label{eq:hgt_attn_review}
\end{equation}
其中 $d$ 為隱藏維度，$H$ 為注意力頭數（本研究設 $H=4$）。

\begin{table}[H]
\centering
\caption{GraphSAGE 與 HGT 的特性比較}
\small
\begin{tabular}{lll}
\toprule
\textbf{面向} & \textbf{GraphSAGE} & \textbf{HGT} \\
\midrule
聚合方式 & 等權鄰域均值 & 多頭注意力加權 \\
鄰居貢獻 & 每個鄰居等量採納 & 根據內容動態加權 \\
歸納學習（新節點） & \textbf{強}（設計核心） & 中 \\
計算複雜度 & 低 & 高（$O(|\mathcal{E}| \cdot H)$）\\
參數量 & 少 & 多 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{各方法對三個核心要求的支援狀況}
\small
\begin{tabular}{lccc}
\toprule
\textbf{方法} & \textbf{上架前預測} & \textbf{全平台指標} & \textbf{不需使用者歷史} \\
\midrule
BPR               & \texttimes & \texttimes & \texttimes \\
LightGCN          & \texttimes & \texttimes & \texttimes \\
MMGCN             & $\triangle$ & \texttimes & \texttimes \\
BM3               & $\triangle$ & \texttimes & \texttimes \\
MMVED / HMMVED    & \checkmark & \checkmark & \checkmark \\
GraphSAGE（本研究預設）& \checkmark & \checkmark & \checkmark \\
HGT（本研究對照）  & \checkmark & \checkmark & \checkmark \\
\bottomrule
\multicolumn{4}{l}{\footnotesize \texttimes：不支援；$\triangle$：部分緩解；\checkmark：完整支援}
\end{tabular}
\end{table}

本研究與 MMVED 的核心差異在於：
本研究額外建立了\textbf{異質圖結構}，讓影片能透過作者節點和分類節點
借用鄰域的歷史表現作為無歷史資料推論的輔助信號，而非僅依賴影片自身的內容特徵。
相較於 LightGCN 等推薦系統 GNN，GraphSAGE 的歸納學習能力使其天然適合
新節點（無歷史資料影片）的推論場景。
...
\item \texttt{interaction.csv}（1.42 GB）：6,767,010 行，28 欄，
      包含使用者、影片、行為與屬性資訊；
\item \texttt{image\_feat.npy}（471 MB）：$153561\times768$ 矩陣，
      ViT-B/16 視覺特徵；
\item \texttt{text\_feat.npy}（61 MB）：$153561\times50$ 矩陣，
      SBERT 文字特徵；
...
\State 儲存 $X \in \mathbb{R}^{54088\times819}$，$Y \in \mathbb{R}^{54088\times8}$
\end{algorithmic}
\end{algorithm}
...
\node[vidnode,draw=vidblue!60,fill=vidblue!30,very thick]
               (vnew) at (-4.2,0) {{\bfseries 新影片}\\ $v_?$（無歷史）};
...
仍可透過 cosine kNN 從視覺相似的老影片借用歷史信號。}
...
\textbf{similar\_to 邊對無歷史資料推論的意義}：
建立 \texttt{similar\_to} 邊時只需影片的視覺特徵（768 維），
不需要任何互動記錄。因此，即使是全新的、無歷史資料的影片，
也能找到 $k=10$ 支視覺上最相似的老影片，
透過 GNN 訊息傳遞從這些老影片的歷史表現中借用信號。
...
\LineComment{節點特徵初始化}
\State video.x $\leftarrow X \in \mathbb{R}^{54088\times819}$
...
\State $F \leftarrow$ 正規化後的視覺特徵矩陣 $\in \mathbb{R}^{54088\times768}$
\State $S \leftarrow F F^\top \in \mathbb{R}^{54088\times54088}$（cosine 相似度）
\For{每部影片 $v$}
...
\node[layerbox] (input)
  {\textbf{輸入層}\\
   \footnotesize video.x $[54088,819]$，author.x $[34939,10]$，category.emb $[51,128]$};
...
\node[layerbox, below=of proj, draw=vidblue!60, fill=vidblue!8] (conv1)
  {\textbf{HeteroConv Layer 1}（GraphSAGE 聚合，1 跳）\\
   \footnotesize 對所有邊類型各自聚合鄰居，加總後 ReLU + Dropout(0.1)};
\node[layerbox, below=of conv1, draw=vidblue!60, fill=vidblue!8] (conv2)
  {\textbf{HeteroConv Layer 2}（GraphSAGE 聚合，2 跳）\\
   \footnotesize 鄰居已融合各自鄰域資訊，感受野擴展到 2 跳；ReLU + Dropout(0.1)};
...
\begin{enumerate}[leftmargin=*]
\item 自身的視覺與文字特徵（$0$ 跳）；
\item 直接鄰居（作者、分類、10 支相似影片）的特徵（$1$ 跳）；
\item 鄰居的鄰居（即 $2$ 跳內所有節點）的特徵（$2$ 跳）。
\end{enumerate}

\subsubsection{對比變體：HGT（異質圖 Transformer）}

\citet{hu2020heterogeneous} 提出的 HGT 以多頭注意力代替均值聚合：
...
\end{equation}
$H$ 為注意力頭數（本實驗設 $H=4$）。
HGT 能學習「哪個鄰居更值得參考」，但計算成本較高，
且在無歷史資料推論能力上略遜於 GraphSAGE。
...
深度學習框架 & PyTorch 2.6，PyTorch Geometric \\
優化器 & Adam，學習率 $10^{-3}$ \\
訓練 epoch & 至多 500，patience=50（早停） \\
隱藏維度 $d$ & 128（可透過 \texttt{--d} 調整） \\
...
這直接驗證了研究假說：
圖神經網路的訊息傳遞能有效地將「視覺風格相似的老影片的觀看時長」
這個上架前唯一可用的鄰域信號，傳遞給新影片的表示向量。
\end{keybox}

Spearman 從 0.047 到 0.256 的提升，代表 HeteroGNN 預測出的影片排名
與真實排名的一致性顯著提高——平台使用這個模型給新影片分配初始流量，
比使用 LightGBM 的準確性高出 4 倍以上。

\subsubsection{核心發現二：Rate 指標的預測天花板}

\begin{sloppypar}
7 個 rate 指標的 Spearman 對兩個模型而言都接近 $0$。
以 \texttt{like\_rate} 為例：84.6\% 的影片按讚率為零，
「哪支影片會爆紅」受到社群網路的隨機傳播效應高度影響，
即「一個大號的轉發就可以帶動按讚數」。
這種\textbf{社群隨機性}無法從影片內容或鄰域信號預測，
兩個模型的 Spearman $\approx 0$ 是誠實且符合預期的結果。
\end{sloppypar}
...
在平台應用中，流量分配關注的是「哪支影片排第一」，
即\textbf{排名能力}（Spearman）更重要。
因此 HeteroGNN 是更合適的解決方案。
MAE 過高的問題可以透過事後輸出校準（isotonic regression 或 Platt scaling）改善，
不需要重新訓練整個模型。

\subsubsection{核心發現四：無歷史資料推論的可行性驗證}

在 from-raw 模式下，使用原始 .mp4 輸入，
系統完整執行：視覺特徵抽取 → cosine kNN 建圖 → HeteroGNN 推論，
驗證了\textbf{端到端無歷史資料推論}的可行性。
全新影片無需任何互動記錄，即可在毫秒級時間內得到 8 個預測指標。
