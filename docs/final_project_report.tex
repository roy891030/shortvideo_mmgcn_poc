...
\textbf{無歷史資料影片}（訓練期未見）& \textbf{14,793} & 任何協同過濾方法均無法預測 \\
\midrule
...
在此高度稀疏的互動矩陣下，協同過濾方法的排名預測能力受到根本性限制，
此為即便是最優基準模型 BM3 其 Recall@10 亦僅達 2.38\% 的核心成因。
\end{definebox}
...
LightGCN 透過圖結構傳遞二跳、三跳以上的高階協同訊號，
...
在表格型資料的回歸與排名任務上表現優異。
與所有協同過濾方法不同，LightGBM 的預測\textbf{僅依賴輸入特徵向量}，
...
表中清楚呈現本研究與 LightGBM 的根本差異：兩者同樣支援上架前表現預測、
全平台指標、以及不需使用者歷史，但 LightGBM 無法利用\textbf{圖鄰域訊號}。
本研究正是透過建立以影片為中心的異質圖，讓 HeteroGNN 能從作者節點和分類節點
借用鄰域的歷史表現作為無歷史資料推論的輔助訊號，
