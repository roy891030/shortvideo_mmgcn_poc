...
\newtcolorbox{tipbox}[1][]{
  colback=tipsbox, colframe=authgreen!70, fonttitle=\bfseries,
  title={#1}, rounded corners, left=4pt, right=4pt}
\newtcolorbox{warnbox2}[1][]{
  colback=warnbox, colframe=hlgold!80, fonttitle=\bfseries,
  title={#1}, rounded corners, left=4pt, right=4pt}
\newtcolorbox{infobox2}[1][]{
  colback=infobox, colframe=vidblue!70, fonttitle=\bfseries,
  title={#1}, rounded corners, left=4pt, right=4pt}
\newtcolorbox{analogybox}[1][]{
  colback=gray!6, colframe=gray!40, fonttitle=\bfseries\itshape,
...
\node[font=\small] at (3.3,-0.4) {86,578 部};
...
\draw[-, draw=authgreen, thick] (0,0)--(0.8,0) node[right, font=\scriptsize] {posted\_by：影片→作者};
\draw[-, draw=catred, thick]    (4,0)--(4.8,0) node[right, font=\scriptsize] {belongs\_to：影片→分類};
...
分類節點 & 51     & 三級分類體系 \\
\midrule
posted\_by 邊（含反向） & 108,176 & 每部影片←→作者 \\
belongs\_to 邊（含反向）& 108,176 & 每部影片←→分類 \\
...
\node[layerbox, below=of proj, draw=vidblue!60, fill=vidblue!8] (conv1)
  ...
  → 影片「知道」作者、分類、相似影片的原始特徵；ReLU + Dropout};
\node[layerbox, below=of conv1, draw=vidblue!60, fill=vidblue!8] (conv2)
  ...
  → 影片的感受野擴展到 2 跳以內的所有節點；ReLU + Dropout};
...
\begin{tabular}{lll}
\toprule
\textbf{面向} & \textbf{GraphSAGE（--model sage）} & \textbf{HGT（--model hgt）} \\
\midrule
